...
- Recommended package-level module export: `import simplecadapi as scad`, then call functions through submodules such as `scad.std.gear.<function>(...)`, `scad.std.chain.<function>(...)`, and `scad.std.bearing.<function>(...)`.
...
- Prefer standard-library factories for standard bearings, spur gears, straight bevel gears, ring gears, and racks before hand-modeling profiles with core geometry APIs.
