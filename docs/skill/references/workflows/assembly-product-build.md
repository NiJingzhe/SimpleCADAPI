...
subassemblies, motion semantics, or durable/cached builds. Not for monolithic
parts (`single-part-modeling.md`) or export-only requests
...
4. **Compose the assembly** in the single assembly file: `@scad.assemble`
...
5. **Express mating and motion as constraints**
...
6. **Solve and read the evidence** (`solve_assembly_constraints_rassembly`,
...
7. **Validate positioning**: component placements realized as intended;
   mating faces flush or offset as specified; axes aligned. Use QL queries on
   the solved assembly, not eyeballing.
8. **Capture** (`domains/assembly-and-product.md`):
   `capture(result, "out/<product>.scadpkg")` — the single delivery boundary.
9. **Export** (`domains/export-and-translation.md`) only after validation.
