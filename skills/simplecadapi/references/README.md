...
This directory is the source of the task-routed skill layer packaged into
`skills/simplecadapi/` by `skill-pack`. It sits between the SDK's generated
API references and the top-level `SKILL.md` router.
...
- Every `references/...` path written here must exist in the packaged skill;
  `skill-pack` validates them (`test_skill_pack.py` enforces the same).
