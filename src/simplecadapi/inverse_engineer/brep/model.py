...
from .io import load_step, shape_mass, xyz
...
def _describe_geometry(
    ...
    *,
    ...
    if kind == "body":
        ...
        volume, centroid = shape_mass(body, "volume")
        area, _ = shape_mass(body, "area")
        return {
            "type": "SOLID",
            "volume": volume,
            "surface_area": area,
    ...
    if kind == "face":
        ...
        area, centroid = shape_mass(face, "area")
        adaptor = BRepAdaptor_Surface(face, True)
        return {
    ...
    if kind == "edge":
        ...
        length, centroid = shape_mass(edge, "length")
        adaptor = BRepAdaptor_Curve(edge)
        return {
    ...
    def summary(
        ...
        *,
        ...
        if has_solid_material:
            ...
            total_volume, centroid = shape_mass(material, "volume")
            material_surface_area, _ = shape_mass(material, "area")
            material_bounding_box = _bounding_box(material)
            material_body_count = _subshape_count(material, TopAbs_SOLID)
        else:
            total_volume, centroid = shape_mass(self.root, "volume")
            material_surface_area, surface_centroid = shape_mass(self.root, "area")
        ...
        for index, body in enumerate(self.bodies):
            volume, center = shape_mass(body, "volume")
            area, _ = shape_mass(body, "area")
...
def load_step_model(path: str | Path) -> BRepModel:
...
def clear_step_model_cache() -> None:
...
def get_model_summary(
    ...
    *,
    ...
    return load_step_model(path).summary(
