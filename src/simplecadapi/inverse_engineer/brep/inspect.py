...
from .io import direction, load_step, shape_mass, xyz
...
def inspect_shape(
    ...
    volume, center = shape_mass(shape, "volume")
    surface_area, _ = shape_mass(shape, "area")
    ...
    for edge_index in range(1, edge_map.Extent() + 1):
        ...
        length, midpoint = shape_mass(edge, "length")
        edges.append(
            {
                "index": edge_index,
...
def inspect_step(path: str | Path) -> BRepInspection:
    """Load and inspect one valid, single-root STEP file."""
    source = Path(path)
    return inspect_shape(load_step(source), source=source)
