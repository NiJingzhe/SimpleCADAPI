...
def _axis_ranges(target: TopoDS_Shape, candidate: TopoDS_Shape, margin_ratio: float):
    target_box = inspect_shape(target).bounding_box
    candidate_box = inspect_shape(candidate).bounding_box
...
def compare_shape_slices(
    ...
    *,
    ...
    selected_slices = (
        tuple(slices) if slices is not None else center_slice_specs(minimum, maximum)
    )
...
def compare_step_slices(
    ...
    **kwargs,
    ...
    return compare_shape_slices(
        load_step(target_source),
        load_step(candidate_source),
