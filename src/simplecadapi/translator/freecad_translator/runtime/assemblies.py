def _make_native_assembly(
    ...
    *,
    ...
    if Assembly is None:
        raise RuntimeError(
            "FreeCAD Assembly workbench module is required for SimpleCAD Assembly translation"
        )
...
def _connector_payload_for(component_entry, connector_id):
    ...
    try:
        ...
        raise RuntimeError(
            f'Missing connector {connector_id!r} on component {component_entry.get("component_id")!r}'
        ) from exc
...
def _connector_reference_for_component(
    assembly_value, component_entry, connector_payload
):
    link = component_entry.get("link")
    if link is None:
        raise RuntimeError(
            f'Missing component link for component {component_entry.get("component_id")!r}'
        )
...
def _make_simplecad_joint(assembly_value, constraint_payload, object_name, label):
    ...
    connector_a_payload = _connector_payload_for(
        component_a, connector_a.get("connector_id")
    )
    connector_b_payload = _connector_payload_for(
        component_b, connector_b.get("connector_id")
    )
    joint_type = {
        "fixed": "Fixed",
        "revolute": "Revolute",
    ...
    }.get(str(constraint_payload.get("constraint_kind")))
    if not joint_type:
        raise RuntimeError(
            f"Unsupported SimpleCAD constraint kind {constraint_payload.get('constraint_kind')!r}"
        )
    ...
    if JointObject is not None:
        ...
        if (
            getattr(App, "GuiUp", False)
            and hasattr(joint, "ViewObject")
            and joint.ViewObject is not None
        ):
            try:
    ...
    try:
        ref_a_obj, ref_a_subs, ref_a_placement, ref_a_detached = (
            _connector_reference_for_component(
                assembly_value, component_a, connector_a_payload
            )
        )
        ref_b_obj, ref_b_subs, ref_b_placement, ref_b_detached = (
            _connector_reference_for_component(
                assembly_value, component_b, connector_b_payload
            )
        )
    ...
    joint.SimpleCADConstraint = json.dumps(
        constraint_payload, ensure_ascii=True, sort_keys=True
    )
...
def _make_simplecad_grounded_joint(assembly_value, component_id):
    ...
    if assembly_container is None:
        raise RuntimeError(
            "Assembly value has no container for grounded joint translation"
        )
    component_entry = _find_component_entry(assembly_value, component_id)
    link = component_entry.get("link")
    if link is None:
        raise RuntimeError(
            f"Missing component link for grounded component {component_id!r}"
        )
    joint_group = _joint_group_for(assembly_container)
    ground = joint_group.newObject(
        "App::FeaturePython", "GroundedJoint_" + str(component_id)
    )
