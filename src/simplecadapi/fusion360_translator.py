...
def _source_kernel_step_payloads(
    ...
    node_ids = list(
        dict.fromkeys(
            _source_kernel_fallback_node_ids(graph)
            + result_boolean_ids
            + detail_input_ids
            + boolean_input_ids
        )
    ...
    for node_id, result in zip(node_ids, results):
        ...
        try:
            ...
            with open(path, "rb") as source:
                compressed = zlib.compress(source.read(), level=9)
...
class Fusion360ScriptTranslator:
    ...
    def translate_model_payload_to_script(
        ...
        *,
        ...
        preferred_ids, active_state, state_node_ids = _preferred_result_node_ids(
            source_graph, self._declared_result_node_id_list
        )
        self._result_state_node_ids = state_node_ids
        self._active_result_state = active_state
        self._result_node_id_list = (
            list(self._explicit_result_node_ids)
...
class SimpleCADFusionRuntime:
    ...
    def _prepare_document(self):
        stale_documents = []
        try:
            # Translation commands are executed in a dedicated benchmark
            # session. Fusion may reopen saved archives as "Untitled", so a
            # name filter leaks documents until Documents.add starts failing.
            stale_documents = [document for document in self.app.documents]
        except Exception:
            stale_documents = []
