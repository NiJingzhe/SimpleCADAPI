...
class AssemblyDefinition:
    ...
    resolved_definitions: Mapping[str, Any] = field(
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(item, PartRef) for item in refs):
            raise ArtifactValidationError(
                "reference_invalid", "/definition_refs", "expected PartRef values"
            )
        ref_ids = [item.definition_id for item in refs]
        if len(set(ref_ids)) != len(ref_ids) or ref_ids != sorted(
            ref_ids, key=lambda item: item.encode("utf-8")
        ):
            raise ArtifactValidationError(
                "array_order_invalid",
                "/definition_refs",
                "refs must be unique and sorted",
            )
        ...
        if len(set(instance_ids)) != len(instance_ids):
            raise ArtifactValidationError(
                "instance_invalid", "/instances", "instance IDs must be unique"
            )
        ...
        for index, relation in enumerate(relations):
            ...
            relation_id = validate_logical_id(
                relation.get("constraint_id"), f"/relations/{index}/constraint_id"
            )
            if relation_id in relation_ids:
                raise ArtifactValidationError(
                    "relation_invalid", f"/relations/{index}", "duplicate constraint ID"
                )
            ...
            for side in ("connector_a", "connector_b"):
                ...
                if not isinstance(value, Mapping) or set(value) != {
                    ...
                    raise ArtifactValidationError(
                        "relation_invalid",
                        f"/relations/{index}/{side}",
                        "invalid connector ref",
                    )
                if value["component_id"] not in valid_instances:
                    raise ArtifactValidationError(
                        "reference_missing",
                        f"/relations/{index}/{side}/component_id",
                        "unknown instance",
                    )
                validate_logical_id(
                    value["connector_id"], f"/relations/{index}/{side}/connector_id"
                )
        object.__setattr__(self, "relations", relations)
        grounded = tuple(self.grounded_instance_ids)
        if len(set(grounded)) != len(grounded) or grounded != tuple(
            sorted(grounded, key=lambda item: item.encode("utf-8"))
        ):
            raise ArtifactValidationError(
                "array_order_invalid",
        ...
        if (
            dict(self.interface_hashes.connectors) != expected_connectors
            or dict(self.interface_hashes.bindings) != expected_bindings
        ):
            raise ArtifactValidationError(
                "interface_invalid",
                "/interface_hashes",
                "public connector hashes differ",
            )
        if self.interface_hashes.material is not None:
            raise ArtifactValidationError(
                "material_invalid",
                "/interface_hashes/material",
                "assembly cannot own material",
            )
        ...
        object.__setattr__(
            self,
            "blobs",
            MappingProxyType({str(k): bytes(v) for k, v in self.blobs.items()}),
        )
        object.__setattr__(
            self,
            "resolved_definitions",
            MappingProxyType(dict(self.resolved_definitions)),
        )
        expected_hash = content_hash(
            self._manifest(content_hash_value="", solved_snapshot=None)
        )
        if (
            self.content_hash
    ...
    def _manifest(
        self, *, content_hash_value: str, solved_snapshot: Mapping[str, Any] | None
    ) -> dict[str, Any]:
        return {
    ...
    def to_dict(self) -> dict[str, Any]:
        return self._manifest(
            content_hash_value=self.content_hash, solved_snapshot=self.solved_snapshot
        )
        ...
        *,
        ...
        return cls(
            ...
            definition_refs=tuple(
                PartRef.from_dict(item, f"/definition_refs/{index}")
                for index, item in enumerate(data["definition_refs"])
            ),
            instances=tuple(
                PartInstance.from_dict(item, f"/instances/{index}")
            ...
            grounded_instance_ids=tuple(
                str(item) for item in data["grounded_instance_ids"]
            ),
            public_connectors=tuple(
                ConnectorInterface.from_dict(item, f"/public_connectors/{index}")
                for index, item in enumerate(data["public_connectors"])
            ),
            interface_hashes=InterfaceHashes.from_dict(data["interface_hashes"]),
            solved_snapshot=(
                dict(data["solved_snapshot"])
