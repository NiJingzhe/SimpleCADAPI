...
const IMGS = {
  assembly: "data:image/png;base64,__IMG_ASSEMBLY__",
  iso: "data:image/png;base64,__IMG_ISO__",
  front: "data:image/png;base64,__IMG_FRONT__",
  shell: "data:image/png;base64,__IMG_SHELL__",
};
(function rendersTab() {
  const caps = { assembly: "最终装配 · body + shell", iso: "等轴测视图", front: "正视图", shell: "外壳 shell（剖视）" };
  const g = $("#rgrid");
  for (const k of ["assembly", "iso", "front", "shell"]) {
    const fig = el("figure");
    const img = new Image();
    img.src = IMGS[k];
    ...
    fig.append(img, el("figcaption", null, caps[k]));
    g.appendChild(fig);
  }
})();
...
(function paramsTab() {
  ...
  t.innerHTML = "<tr><th>参数</th><th>终值</th><th>说明</th></tr>" +
    rows.map(r => "<tr><td class='k'>" + r[0] + "</td><td class='v'>" + r[1] + "</td><td class='c'>" + r[2] + "</td></tr>").join("");
  $("#tab-params .panebody").appendChild(t);
  const tags = el("div");
  tags.innerHTML = '<div class="secttl">确定性命名面（QL 可索引）</div>' +
    '<span class="tagchip">feature.motor_mount_floor_left</span>' +
    '<span class="tagchip">feature.motor_mount_floor_right</span>' +
    '<span class="tagchip">feature.back_face</span>' +
    '<p style="margin-top:8px;color:var(--ink-faint);font-size:11.5px">命名面在倒角、剖分、重打标后仍可被查询语言索引 —— 这是本轮会话的核心能力演示之一。</p>';
...
(function outTab() {
  ...
  const vscripts = ["s1–s5_verify", "s6_param_sweep", "s8_preprobe/diag×3", "s9_diag/verify", "s10_verify", "s11_probe", "s12_verify", "s13_probe", "s14 回归"];
  $("#tab-out .panebody").innerHTML =
    '<div class="secttl">导出产物 · out/</div><div class="filelist">' +
    files.map(f => "<div><b>" + f[0] + "</b> <span class='sz'>" + f[1] + "</span> — " + f[2] + "</div>").join("") +
    '</div><div class="secttl">验证与探针脚本 · verify/</div><div class="filelist">' +
    vscripts.map(s => "<div>" + s + ".py</div>").join("") +
    '</div><div class="secttl">会话统计</div><div class="filelist">' +
    ...
    "<div>参数扫描 <b>13 变体 + 17 边界</b> 全过</div></div>";
})();
(function aboutTab() {
  $("#tab-about .panebody").innerHTML =
    '<div class="about"><p><b>这是什么。</b>一个真实工程会话的忠实回放：2026-08-26 15:56 → 00:17，人类工程师用自然语言描述需求、逐步定向修改，Agent（SimpleCAD SDK）在 <b>' + STATS.toolTotal + '</b> 次工具调用中完成「需求确认 → 总体规划 → 分阶段建模 → 每阶段独立验证 → 导出装配」的完整闭环。</p>' +
    '<p><b>左侧对话</b>逐条回放全部 ' + N + ' 条消息（用户输入 / Agent 思考与回复 / 工具调用输入与输出 / 补丁 / 角色切换），内容与 session_transcript.md 一一对应，未做删改。</p>' +
    '<p><b>右侧 3D</b> 是会话最终导出的 u_link_assembly.stl（' + (44976).toLocaleString() + ' 三角面），由 three.js 实时渲染，可拖拽旋转 / 缩放 / 平移。</p>' +
    '<p><b>操作</b>：<span class="kbd">空格</span> 播放/暂停 · <span class="kbd">⇧→</span> 跳到下一轮用户输入 · 点击进度条橙色刻度直达对应轮次 · 速度可调（×0.5–∞）·「阅读模式」一次渲染全部。</p>' +
    '<p><b>看点</b>：① 每阶段先写验证契约再建模（' + STATS.allPass + ' 次 ALL PASS）；② 安装面/背面具备确定性命名、可被查询语言索引；③ 用户 7 次「不对/改一下」的真实返工（S8 卡扣→S9 boss→S10 剖分→S11 两刀工艺）；④ 参数守卫链与变体扫描保证参数化正确性。</p>' +
    '<p style="color:var(--ink-faint);font-size:11.5px">会话 ' + esc(META.id) + ' · 由 demo/src/build_demo.py 从 transcript 自动生成本页</p></div>';
})();
...
(function sessionCard() {
  ...
  $("#sessionCard").innerHTML =
    "<h2>" + esc(META.title) + " <span style='color:var(--accent2);font-size:12px'>· 从一句话需求到可制造装配</span></h2>" +
    ...
    '<div class="stat"><b>S1–S14</b><span>建模阶段</span></div>' +
    ...
    '<div class="stat"><b>2</b><span>交付零件 body+shell</span></div>' +
    "</div>";
  $("#metaLine").textContent = META.title + " · " + STATS.userTurns + " 轮用户输入 · " + STATS.toolTotal + " 次工具调用 · S1–S14";
})();
...
function glInit() {
  try {
    ...
    scene.add(rim);
    const geo = new THREE.STLLoader().parse(b64ToBuf("__STL_B64__"));
